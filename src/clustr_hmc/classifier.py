...
from clustr_hmc import traverse as tr
import numpy as np
...
from clustr_hmc.decorators import logger
from clustr_hmc.dummy import DummyProgress
from clustr_hmc.graph import make_flat_hierarchy, rollup_nodes
...
class HierarchicalClassifier(BaseEstimator, ClassifierMixin, MetaEstimatorMixin):
    ...
    - Local classifiers - the local (or "base") classifiers can theoretically be chosen to be of any kind, but we
        distinguish between three main modes of local classification:
            ...
            * "One classifier per node" - where each node is fitted with a binary "membership" classifier which
                ...
                example.
            * "One Classifier per level" = Where each level is fitted with a classifier
            * Global / "big bang" classifiers - where a single classifier predicts the full path in the hierarchy
                for a given example. CURRENTLY NOT BEING IMPLEMENTED
    ...
    base_estimator : classifier object, function, dict, or None
        ...
        If not provided, a base estimator will be chosen by the framework using a linear
    ...
    algorithm : "lcn", "lcpn", "lcpl"
        ...
        "lcl" which stands for "local classifier per level" Under this a classifier is trained at each level
    ...
    label : string
        can be 'leaf' or 'path'. leaf is when only the leaf nodes are present. 'path' is when the full path is
        provided.
    ...
    multi_label : True or False. By default True.
        Whether leaf nodes can be assigned. Please note that this is different from
    ...
    def __init__(
        ...
        self.transform_label = do_transform_label
        self.label_transformer = label_transformer
        self.include_parent = include_parent
        self.mlb_model = mlb_model
        self.training_stopping_criteria = training_Stopping_criteria

    def fit(self, X, y=None, sample_weight=None, label_to_path=True):
        ...
        label_to_path : If the labels are leaf nodes or most specific class and path needs to be inferred
            True/False
        ...
        self
        """
        assert X.shape[0] == y.shape[0], "Number of rows dont match"
        ...
        pre_label = pre.DataPreprocess(type=self.algorithm)
        if self.label == 'leaf':
            y_paths = pre_label.preprocess_label(root=ROOT, G=self.graph_,labels=y)
            self.graph_ = pre_label.assign_nodes(G=self.graph_, labels=y_paths)
        elif self.label == 'path':
            self.graph_ = pre_label.assign_nodes(G=self.graph_, labels=y)
        else:
            raise ValueError('Unknown label value')

        if isinstance(self.training_strategy, str):
            policy = [x for x in dir(pre_label) if re.search(self.training_strategy, x)]
            if len(policy) > 0:
                policy = getattr(pre_label, policy[0])
            else:
                policy = None
        else:
            policy = self.training_strategy
        # Recursively train base classifiers
        #with self._progress(total=self.n_classes_ + 1, desc="Training base classifiers") as progress:
        train = tr.TrainTraverse(algo=self.algorithm, G=self.graph_, X=X, local_model=self.base_estimator,
                                 path_labels=y_paths, is_tree=self.is_tree, is_multi_label=self.multi_label,
                                 do_transform=self.transform_label, label_transformer=self.label_transformer,
                                 include_parent=self.include_parent, policy=policy, mlb_model=self.mlb_model,
                                 )

        return self

    def predict(self, X):
        """Predict multi-label-hierarchical targets using underlying estimators.
    ...
    def _recursive_build_features(self, X, y, node_id, progress):
        ...
        if self.graph_.out_degree(node_id) == 0:
            # Leaf node
            non_zeros = [np.flatnonzero(y[i, :]) for i in range(y.shape[0])]
            non_zeros_labels = [[self.mlb.classes_[i] for i in arr] for arr in non_zeros]
            indices = [i for i, x in enumerate(non_zeros_labels) if node_id in x]
    ...
    def _build_metafeatures(self, X, y):
        ...
        non_zeros = [np.flatnonzero(y[i, :]) for i in ix]
        non_zeros_labels = [[self.mlb.classes_[i] for i in arr] for arr in non_zeros]

        return dict(
            n_samples=len(ix),
            n_targets=len(np.unique(non_zeros_labels)),
        )
    ...
    def _train_local_classifier(self, X, y, node_id):
        ...
        if self.is_tree_:
            ...
            if self.feature_extraction == "raw":
                ...
                mlb_for_classification = MultiLabelBinarizer()
                y_ = mlb_for_classification.fit_transform(y_rolled_up)

        num_targets = len(np.unique([y_l for y_d in y_rolled_up for y_l in y_d]))
        ...
        if self.feature_extraction == "raw":
            ...
            clf.fit(X=X_, y=y_)
            self.graph_.nodes[node_id][CLASSIFIER] = clf
            self.graph_.nodes[node_id][CLASSIFIER + '_MLB'] = mlb_for_classification
    ...
    def _recursive_predict(self, x, root):  # noqa:C901 TODO: refactor
        ...
        clf_mlb = self.graph_.nodes[root][CLASSIFIER+"_MLB"]
        path = []
        path_proba = []
        ...
        while clf:
            ...
            args = np.where(probs[0] > self.mlb_prediction_threshold)[0]
            scores = probs[0,args]

            path_proba.append(scores)
            predictions = []

            # Report probabilities in terms of complete class hierarchy
            if len(clf_mlb.classes_) == 1:
                predictions = [clf_mlb.classes_[0]]

            for local_class_idx, class_ in enumerate(clf_mlb.classes_):
                if self.mlb:
                    # when we have a multi-label binarizer
                    #print(class_)
                    class_idx = np.where(np.array(self.classes_) == class_)[0][0]
                    ...
                    if preds_prob is not None:
                        ...
                        for p in pred_path:
                            if root != self.root:
                                ...
                                path.append(p)
                if not path and root != self.root:
                    path = [[root]]
